...
void interactive_hex_dump(const std::vector<unsigned char>& data) {
    ...
    while (current_offset < data.size())
    {
        ...
        const size_t end_offset = std::min(current_offset + PAGE_SIZE, data.size());
...
int main() {
    ...
    const std::vector<unsigned char> content = read_file(filename);
    ...
    const std::string target_string = "Enter the key: ";
